...
DEFAULT_SUGGESTIONS = [
    "どのテーブルがあり、どのように関連していますか？要点だけ簡潔にまとめてください。",
    "どの配送センターがボトルネックになる可能性が最も高いですか？",
    "このデータセットの内容と前提条件を説明してください。",
    "2024年の週次で、製品別の需要はどう推移しましたか？"
]
...
def call_llm_for_insights(df, prompt=None):
    ...
    """
    # Default instruction in Japanese
    if prompt is None:
        prompt = (
            "あなたはプロのデータアナリストです。以下のテーブルデータに基づいて、"
            "1) 重要な洞察とトレンド、2) 顕著なパターンや異常、3) ビジネス上の示唆の3点を、"
            "箇条書き中心で、簡潔かつ具体的に解説してください。出力は必ず日本語で、Markdownを用いて読みやすく構成してください。\n\n"
        )
    # Ensure Japanese output requirement even when a custom prompt is provided
    if "日本語" not in prompt:
        prompt += "出力は必ず日本語で、Markdownで簡潔にまとめてください。\n\n"

    csv_data = df.to_csv(index=False)
    full_prompt = f"{prompt}テーブルデータ:\n{csv_data}"
    try:
        client = WorkspaceClient()
        # Prefer using a system message if supported by the serving endpoint
        try:
            system_role = ChatMessageRole.SYSTEM
        except AttributeError:
            system_role = ChatMessageRole.USER
        system_content = (
            "あなたは熟練したデータアナリストです。常に日本語で、過度に長文化せず、"
            "具体的かつ実務に役立つ示唆をMarkdownで出力してください。"
        )
        messages = []
        if system_role != ChatMessageRole.USER:
            messages.append(ChatMessage(content=system_content, role=system_role))
        else:
            # If SYSTEM role unsupported, prepend the instruction in the user content
            full_prompt = system_content + "\n\n" + full_prompt
        messages.append(ChatMessage(content=full_prompt, role=ChatMessageRole.USER))
