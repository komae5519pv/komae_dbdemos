...
if not SPACE_ID:
    logger.warning("SPACE_ID is not set. Set SPACE_ID in environment or app.yaml.")
if not DATABRICKS_HOST:
    logger.warning("DATABRICKS_HOST is not set. This should be auto-injected in Databricks Apps or set locally in .env.")
...
def start_new_conversation(question: str) -> Tuple[str, Union[str, pd.DataFrame], Optional[str]]:
    ...
    try:
        ...
        if status in (401, 403, 404):
            guidance = (
                "Permission or visibility issue accessing the Genie space. "
                "In Databricks Apps, grant 'can_run' to the app's execution identity (service principal) "
                f"on the space (ID: {SPACE_ID}). 404 can also occur when access is denied. "
                "Additionally ensure 'can_use' on the SQL warehouse and proper data privileges."
            )
            logger.error(f"Genie permission/visibility error (status {status}): {http_err}")
            return None, guidance, None
        elif status == 429:
            return None, "System is busy (429). Please try again shortly.", None
        else:
            logger.error(f"HTTP error during start_conversation: {http_err}")
            return None, f"Sorry, an error occurred: {str(http_err)}. Please try again.", None
...
def continue_conversation(conversation_id: str, question: str) -> Tuple[Union[str, pd.DataFrame], Optional[str]]:
    ...
    try:
        ...
        if status in (401, 403, 404):
            guidance = (
                "Permission or visibility issue accessing the Genie space. "
                "Grant 'can_run' to the app's execution identity (service principal) on the space, "
                f"space ID: {SPACE_ID}. Also ensure warehouse 'can_use' and data privileges."
            )
            logger.error(f"Genie permission/visibility error on continue (status {status}): {http_err}")
            return guidance, None
        elif status == 429:
            return "Sorry, the system is currently experiencing high demand. Please try again in a few moments.", None
        elif status == 404 and "Conversation not found" in str(http_err):
            return "Sorry, the previous conversation has expired. Please try your query again to start a new conversation.", None
        else:
            logger.error(f"HTTP error during continue_conversation: {http_err}")
            return f"Sorry, an error occurred: {str(http_err)}", None
    except Exception as e:
